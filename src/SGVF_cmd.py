...
def _addargs(parser):
    parser.add_argument('input_glob', help = 'A glob string that includes all the files processed with the PerFile_cmd', required = True)
    parser.add_argument('output_dsgv', help = 'Output path for the deletion-sgv dataframe. By default a pickled pandas dataframe', required = True)
    parser.add_argument('output_vsgv', help = 'Output path for the variable-sgv dataframe. By default a pickled pandas dataframe', required = True)
...
if __name__ == '__main__':
    ...
    if args.byother:
        vsgv, dsgv = calculate_by_other(join(split(realpath(__file__))[0], 'orig_dsgv.df'),
                           join(split(realpath(__file__))[0], 'orig_vsgv.df'),
                           join(split(realpath(__file__))[0], 'orig_frames'), 
